...
//
// __STL_STATIC_CONST_INIT_BUG implies QUARK_NO_LIMITS_COMPILE_TIME_CONSTANTS
...
#  if !(_STLPORT_VERSION >= 0x500) && !defined(_STLP_OWN_IOSTREAMS) && defined(_STLP_USE_NAMESPACES) && defined(QUARK_NO_USING_TEMPLATE) && !defined(__BORLANDC__)
#     define QUARK_NO_STD_LOCALE
#  endif
#else
#  if !defined(__SGI_STL_OWN_IOSTREAMS) && defined(__STL_USE_NAMESPACES) && defined(QUARK_NO_USING_TEMPLATE) && !defined(__BORLANDC__)
#     define QUARK_NO_STD_LOCALE
#  endif
#endif
...
#if defined(_STLP_NO_MEMBER_TEMPLATE_KEYWORD) && defined(QUARK_MSVC) && (QUARK_MSVC <= 1300)
#  define QUARK_NO_STD_ALLOCATOR
#endif
...
#  ifndef  QUARK_NO_STD_WSTRING
#     define QUARK_NO_STD_WSTRING
#  endif
#  ifndef  QUARK_NO_STD_WSTREAMBUF
...
// but doesn't always get them all, define QUARK_NO_STDC_NAMESPACE, since our
// workaround does not conflict with STLports:
//
//
// Harold Howe says:
// Borland switched to STLport in BCB6. Defining QUARK_NO_STDC_NAMESPACE with
// BCB6 does cause problems. If we detect C++ Builder, then don't define 
// QUARK_NO_STDC_NAMESPACE
//
#if !defined(__BORLANDC__) && !defined(__DMC__)
//
// If STLport is using it's own namespace, and the real names are in
// the global namespace, then we duplicate STLport's using declarations
// (by defining QUARK_NO_STDC_NAMESPACE), we do this because STLport doesn't
...
#     define QUARK_NO_EXCEPTION_STD_NAMESPACE
#  endif
...
#  define QUARK_HAS_STLP_USE_FACET
#endif

//
// If STLport thinks there are no wide functions, <cwchar> etc. is not working; but
// only if QUARK_NO_STDC_NAMESPACE is not defined (if it is then we do the import 
// into std:: ourselves).
//
#if defined(_STLP_NO_NATIVE_WIDE_FUNCTIONS) && !defined(QUARK_NO_STDC_NAMESPACE)
...
//
#if !defined( _STLP_HAS_WCHAR_T) || defined(_STLP_WCHAR_T_IS_USHORT)
#  undef  QUARK_NO_INTRINSIC_WCHAR_T
...
#  define QUARK_USING_STD_MIN() ((void)0)
#  define QUARK_USING_STD_MAX() ((void)0)
namespace quark { using std::min; using std::max; }
#endif
...
#define QUARK_STDLIB "STLPort standard library version " QUARK_STRINGIZE(__SGI_STL_PORT)
